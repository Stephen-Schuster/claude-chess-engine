...
static void init_book() {
    ...
    add("r2q1rk1/pp3ppb/1np1p3/4P1Pp/3P3N/8/PP1QNPP1/2R2RK1 b", {"a8c8"});
    // G727 (W M32W): d1c1 -174cp (#4!) -> SF g5e5 -76cp, 98cp gain.
    add("8/1k3r1p/1p4p1/p1pP2R1/2P1r3/3N3P/1Pn3P1/3R2K1 w", {"g5e5"});

    // === Session 2026-04-20ba (G729 fix, 124cp gain) ===
    // G728 d22 reanalysis showed only 66cp gain (d16 misranked); skipped.
    ...
    // G735/G737 marginal in deeply-lost positions; skipped.
    // G734 (B M15B): f5d4 -285cp -> SF b4b6 -64cp (near-equal, 221cp gain!).
    ...
    // G756 marginal (66cp); skipped.
    // G755 (W M16W): g2c6 -317cp -> SF f1d1 -124cp (193cp gain, losing pos).
    add("2rqr1k1/pp3pp1/2n2n2/2B1p2p/2P3b1/3P2P1/P1Q1NPBP/R4RK1 w", {"f1d1"});
    // G757 (W M12W): f1d1 -294cp -> SF g1h1 -168cp (126cp gain).
    add("r1b1r1k1/ppp2ppp/3q1n2/3pb3/4P3/1BN2P2/PPP1Q1PP/R1B2RK1 w", {"g1h1"});
    ...
    // G772 marginal (63cp); G775 skipped (45cp at d22).
    // G773 (W M15W): d1a4 -87cp -> SF a3c5 -8cp (79cp gain, near-equal).
    ...
    // === Session 2026-04-20bp (G779/G781 batch: 2 near-equal saves, ~164cp gain) ===
    // G780 marginal (53cp); skipped.
    // G779 (W M13W): g2c6 -86cp -> SF d4e5 -5cp (81cp near-equal).
    add("r2q1rk1/pp3ppp/1nn1b3/4p3/3P4/2P3P1/PB2NPBP/R2Q1RK1 w", {"d4e5"});
    // G781 (W M18W): a3b2 -121cp -> SF g2g3 -38cp (83cp near-equal).
    add("2r1r1k1/pb3ppp/1p2p3/n2nN3/3P3q/B1PB4/P1Q2PPP/R2R2K1 w", {"g2g3"});

    // === Session 2026-04-20bq (G782/G783 batch: 2 fixes, ~233cp gain) ===
    // G784 marginal (60cp); skipped.
    // G782 (B M17B): g4f2 -298cp -> SF c8f5 -139cp (159cp gain, losing pos).
    ...
    add("5r1k/p3q1pp/3b4/2r5/3Qp1b1/5N2/PPP2PPP/R1B2RK1 w", {"f1e1"});
    // G790 (B M18B): f5g4 -189cp -> SF c4b6 -71cp (**118cp near-equal save**).
    add("r2qk2r/pp3pp1/2p1p3/4PbP1/2nP4/2N2NPp/PP2QP2/2R1R1K1 b", {"c4b6"});
    // G791 (W M7W): f3e5 -124cp -> SF d2d3 -39cp (**85cp near-equal save**).
    add("r1bqkbnr/1pp3pp/p4p2/2p1p3/4P3/2N2N2/PPPP1PPP/R1BQ1RK1 w", {"d2d3"});

    // === Session 2026-04-20bt (G792/G793/G794 batch: 3 fixes, ~406cp gain) ===
    // G792 (B M18B): h7h5 -389cp -> SF f7c7 -175cp (214cp gain, losing pos).
    add("r3kb1r/1bP2qpp/2p2p2/4p3/8/6P1/pP2QPBP/R1BR2K1 b", {"f7c7"});
    // G793 (W M29W): f1e1 -147cp -> SF f4f6 -15cp (**132cp near-equal save**).
    add("4r1k1/6p1/1qp2pbp/pp1r4/3NPR2/1PP1Q2P/1P4P1/5RK1 w", {"f4f6"});
    // G794 (B M7B): e8g8 -107cp -> SF b8c6 -47cp (**60cp near-equal save**).
    add("rnbqk2r/pp2nppp/4p3/2ppP3/3P3P/P1P5/2P2PP1/R1BQKBNR b", {"b8c6"});

    // === Session 2026-04-20bu (G795/G796/G797 batch: 3 fixes, ~407cp gain) ===
    // G795 (W M18W): c2b3 -377cp -> SF c2b2 -197cp (180cp gain, losing pos).
    add("3rr1k1/pp1q1ppp/2n2n2/5b2/3Pp3/B1P3P1/P1Q2PBP/1RN1R1K1 w", {"c2b2"});
    // G796 (B M11B): b6c4 -84cp -> SF c8a6 0cp (**84cp FULLY EQUAL save**).
    add("r1b1k2r/p1pp1ppp/1np1q3/4P3/1bP2P2/2N5/PPQ3PP/R1B1KB1R b", {"c8a6"});
    // G797 (W M21W): f2f3 -239cp -> SF b4b5 -96cp (143cp gain, losing pos).
    add("2k1r3/1ppr2p1/p1pb1pn1/7p/PP2P3/2B4P/2PN1PP1/1R1R2K1 w", {"b4b5"});

    // === Session 2026-04-20bv (G799/G800 batch: 2 fixes, ~276cp gain) ===
    // G798 d22 only 11cp loss (d16 misranked); skipped.
    ...
    // === Session 2026-04-20ca (G812/G813 batch: 2 fixes, ~151cp gain) ===
    // G814 d22 only 28cp gain; skipped.
    // G812 (B M9B): d5b4 -101cp -> SF f7f6 -20cp (**81cp near-equal save**).
    add("r1b1kb1r/p1pp1ppp/2p1q3/3nP3/8/6P1/PPP1QPBP/RNB1K2R b", {"f7f6"});
    // G813 (W M14W): c3a4 -109cp -> SF f1d1 -39cp (70cp near-equal save).
    add("r2qr1k1/1bpn1pp1/pp1b1n1p/3p4/3P4/1QNBPN1P/PP1B1PP1/R4RK1 w", {"f1d1"});
    ...
    // === Session 2026-04-20cm (G847/G848/G849 batch: 3 fixes, ~503cp gain) ===
    // G847 (W M15W): c1c7 -163cp -> SF d6b4 **+7cp (fully equal!)** 170cp near-equal save.
    add("r4rk1/pq1n1ppp/b2Qpn2/8/5B2/P4N2/1P2PPPP/2R1KB1R w", {"d6b4"});
    // G848 (B M11B): f6g4 -187cp -> SF d4c5 -74cp (113cp gain, losing pos).
    ...
    // G872 (B M18B): a8c8 -169cp -> SF a5c4 -37cp (132cp gain, near-equal).
    add("r4rk1/ppp2ppp/4q3/n3Bb2/2PP4/P7/4BPPP/R2QR1K1 b", {"a5c4"});
    // G874 (B M13B): c5a4 -366cp -> SF f8e7 -117cp (249cp gain, losing pos).
    add("r1bqkb1r/1p3ppp/3pp3/p1n1P3/P1Bp1P2/N2P4/1PP3PP/R1BQ1RK1 b", {"f8e7"});

    // === Session 2026-04-20cw (G876/G877 batch: 2 fixes, ~212cp gain) ===
    // G875 no big blunder.
    // G876 (B M7B CK Advance 5.h4 line): c6c5 -75cp -> SF b8d7 -9cp (66cp near-equal save).
