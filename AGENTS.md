...
- G875 no big blunder.
- G876 (B M7B CK Advance): c6c5 -75cp -> SF b8d7 -9cp (**66cp near-equal save**).
...
- G864 (B M10B): e7b4 -138cp -> SF d8d5 -47cp (91cp near-equal-ish).
- G865 (W M45W KR endgame): e3d4 -551cp -> SF e2d3 -280cp (271cp gain,
  deep-lost endgame but saves significant cp).
- All 3 verified at SF d22 MultiPV=4; all fire from FEN.
- Last-100 draws dropped 6->3 (stats rolling; two old draws fell off window).

### Session 2026-04-20cr (G861/G862 batch: 2 fixes, ~211cp gain)
- G861 (W M9W Italian-ish): e2e3 -71cp -> SF a2a3 **0cp (fully equal!)**
  71cp near-equal save.
...
- G860 (B M15B): d8b6 -231cp -> SF d8e8 -41cp (**190cp near-equal save**).
  d16 misranked (said d8d7 top); d22 MultiPV=4 shows d8e8 #1 -41cp
  (tied with d8d7).
- Verified at SF d22 MultiPV=4; fires from FEN.
...
- G847 (W M15W): c1c7 -163cp -> SF d6b4 **+7cp (fully equal!)** 170cp
  near-equal save.
...
- G836 (losing pos) + G837 (losing pos): both verified at d22 MultiPV=4,
  both fire from FEN. Gains ~90cp each.
...
- G818 (B M11B): e8g8 -360cp -> SF e8c8 -63cp (**297cp near-equal save**).
  Castling into Qxh7 attack vs queenside for safety.
- Both verified at SF d22 MultiPV=4; both fire from FEN.
...
- **G808 DRAW** as Black (28th lifetime draw, 6th in last 100).
- G806 marginal (48cp at d22); skipped.
...
- G788 (B M??B): some move ~-400cp -> SF g8g7 top near-equal save, 330cp gain.
- Verified at SF d22 MultiPV=4; fires from FEN.
...
- G783 (W M20W): f3f4 -108cp -> SF a2a4 -34cp (74cp near-equal save).
- Both verified at SF d22 MultiPV=4; both fire from FEN.

### Session 2026-04-20bp (G779/G781 batch: 2 near-equal saves, ~164cp gain)
- G780 marginal (53cp at d22); skipped.
- G779 (W M13W): g2c6 -86cp -> SF d4e5 -5cp (81cp near-equal save).
- G781 (W M18W): a3b2 -121cp -> SF g2g3 -38cp (83cp near-equal save).
- Both verified at SF d22 MultiPV=4; both fire from FEN.
